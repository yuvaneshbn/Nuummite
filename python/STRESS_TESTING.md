...
- Quick smoke test (matches current debug log format): `python python/udp_client_stress.py --clients 40 --seconds 5 --loss 5 --processes 4 --debug-audio`
...
- Mixed peak vs. client count: `pre_limiter_peak` can exceed 32767 when many clients are summed (that’s expected); limiter/soft-clip should keep the output bounded.
...
- You should see periodic lines like:
  - `[AUDIO] active_clients=40 pre_limiter_peak=... queues=[0,1,2,...]`
  - `[AUDIO] plc_lost_frames=1 sender=c0002`
- Healthy run: most per-client queue depths stay around 0–3, and PLC events are occasional/small (1–2 frames).
