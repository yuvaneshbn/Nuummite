...
# Pre-compiled UI modules (generated by pyside6-uic).
from python.ui.main_window_ui import Ui_MainWindowForm
from python.ui.participant_item_ui import Ui_ParticipantItemForm
from python.ui.popup_message_ui import Ui_Dialog
from python.ui.settings_dialog_ui import Ui_SettingsDialogForm
...
def _ui_build_widget(widget: QtWidgets.QWidget, ui) -> QtWidgets.QWidget:
    ui.setupUi(widget)
    return widget


def _find_and_replace_in_layout(
    ...
    for index in range(layout.count()):
        ...
        if sub_layout is not None and _find_and_replace_in_layout(sub_layout, old_widget, new_widget):
            return True

    return False


def _replace_sliders_with_tick(root: QtWidgets.QWidget) -> None:
    # Replace designer-created QSliders with TickSlider to preserve tick rendering under QSS.
    # We do this post-setupUi to keep objectName() stable for findChild() lookups.
    # Only replace exact QSlider instances (avoid replacing already-replaced TickSlider subclasses)
    for old in list(root.findChildren(QtWidgets.QSlider)):
        if type(old) is not QtWidgets.QSlider:
            continue
        parent_widget = old.parentWidget()
        if parent_widget is None:
            continue
        if isinstance(parent_widget, VolumeControlWidget):
            continue
        replacement = TickSlider(parent_widget)
        replacement.setObjectName(old.objectName())
        replacement.setOrientation(old.orientation())
        replacement.setMinimum(old.minimum())
        replacement.setMaximum(old.maximum())
        replacement.setSingleStep(old.singleStep())
        replacement.setPageStep(old.pageStep())
        replacement.setTickPosition(old.tickPosition())
        replacement.setTickInterval(old.tickInterval())
        replacement.setValue(old.value())

        replaced = False
        layout = parent_widget.layout()
        if layout is not None:
            replaced = _find_and_replace_in_layout(layout, old, replacement)
        if not replaced:
            replacement.setParent(parent_widget)
            replacement.setGeometry(old.geometry())
            replacement.show()
        old.deleteLater()


def ui_popup_dialog(parent=None) -> QtWidgets.QDialog:
    dlg = _ui_build_widget(QtWidgets.QDialog(parent), Ui_Dialog())
    _replace_sliders_with_tick(dlg)
    return dlg


def ui_main_window_form(parent=None) -> QtWidgets.QWidget:
    w = _ui_build_widget(QtWidgets.QWidget(parent), Ui_MainWindowForm())
    _replace_sliders_with_tick(w)
    return w


def ui_participant_item_form(parent=None) -> QtWidgets.QWidget:
    w = _ui_build_widget(QtWidgets.QWidget(parent), Ui_ParticipantItemForm())
    _replace_sliders_with_tick(w)
    return w


def ui_settings_dialog_form(parent=None) -> QtWidgets.QWidget:
    w = _ui_build_widget(QtWidgets.QWidget(parent), Ui_SettingsDialogForm())
    _replace_sliders_with_tick(w)
    return w


def ui_volume_control_form(parent=None) -> QtWidgets.QWidget:
    w = _ui_build_widget(QtWidgets.QWidget(parent), Ui_VolumeControlForm())
    _replace_sliders_with_tick(w)
    return w
...
class ParticipantRow(QtCore.QObject):
    ...
    def __init__(self, client_id: str, is_self: bool, talk_checked: bool, mute_checked: bool, parent=None):
        ...
        self.widget = ui_participant_item_form()
...
class VolumeControlPanel(QtCore.QObject):
    def __init__(self, audio: PyAudioEngine, parent=None):
        ...
        self.widget: QtWidgets.QWidget = ui_volume_control_form()
        try:
            # Clear Python-side references when the underlying Qt widget is destroyed
    ...
    def refresh_from_settings(self) -> None:
        ...
        echo_on = bool(self.echo_checkbox and self.echo_checkbox.isChecked())
        self._sync_feature_controls()
        self.set_mic_level(self.audio.capture_level)
        try:
            if self.gain_label:
                try:
                    self.gain_label.setEnabled(not autogain_on)
                    self.gain_label.setText(
                        f"Gain (dB): {self.gain_slider.value() if self.gain_slider else ''}"
                        + (" (Auto Gain enabled)" if autogain_on else "")
                    )
                except RuntimeError:
                    pass
        except RuntimeError:
    ...
    def _apply_ui_to_engine(self) -> None:
        def _safe_value(widget, default=0):
            if not widget:
                return default
            try:
        ...
        def _safe_checked(widget, default=False):
            if not widget:
                return default
            try:
        ...
        try:
            self.audio.set_master_volume(_safe_value(self.master_slider, 100))
        except Exception:
            pass
        if self.output_slider:
            try:
                self.audio.set_output_volume(_safe_value(self.output_slider, 100))
            except Exception:
                pass
        try:
            self.audio.set_gain_db(_safe_value(self.gain_slider, 0))
        except Exception:
            pass
        try:
            self.audio.set_noise_suppression(_safe_value(self.ns_slider, 65))
        except Exception:
            pass
        if self.input_slider:
            try:
                self.audio.set_mic_sensitivity(_safe_value(self.input_slider, 45))
            except Exception:
                pass
        if self.aec_delay_slider:
            try:
                self.audio.set_aec_stream_delay_ms(_safe_value(self.aec_delay_slider, 180))
            except Exception:
                pass
        try:
            self.audio.set_auto_gain(_safe_checked(self.autogain_checkbox, False))
        except Exception:
            pass
        try:
            self.audio.set_noise_suppression_enabled(_safe_checked(self.rnnoise_checkbox, False))
        except Exception:
            pass
        try:
            self.audio.set_echo_enabled(_safe_checked(self.echo_checkbox, False))
        except Exception:
            pass
    ...
    def _on_widget_destroyed(self):
        # Called when the Qt widget is deleted; drop references to avoid RuntimeError
        try:
            attrs = [
                'master_label', 'master_slider', 'output_label', 'output_slider', 'gain_label', 'gain_slider',
                'ns_label', 'ns_slider', 'aec_delay_label', 'aec_delay_slider', 'input_label', 'input_slider',
                'test_button', 'test_status', 'mic_bar', 'reset_button', 'autogain_checkbox', 'rnnoise_checkbox', 'echo_checkbox'
            ]
            for a in attrs:
                try:
                    setattr(self, a, None)
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, audio: PyAudioEngine, server_ip: str, parent=None):
        ...
        self.settings = QtCore.QSettings()
        self.form = ui_settings_dialog_form()
        ...
        self.volume_controls = self.form.findChild(VolumeControlWidget, "advancedAudioControl")
        if self.volume_controls is None:
            raise RuntimeError("Promoted advancedAudioControl widget not found in settings dialog UI")
        self.volume_controls.initialize_audio(audio)
        self.volume_controls.refresh_from_settings()
...
def apply_saved_audio_settings(audio: PyAudioEngine) -> None:
    ...
    if in_idx is not None:
        try:
            log.info("Applying audio/inputDeviceIndex=%s", in_idx)
            audio.set_input_device(in_idx)
        except Exception:
            pass
    if out_idx is not None:
        try:
            log.info("Applying audio/outputDeviceIndex=%s", out_idx)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, my_id: str, room_name: str, audio: PyAudioEngine, discovery: PyPeerDiscovery):
        ...
        self.volume_controls = find(VolumeControlWidget, "advancedAudioControl")
        if self.volume_controls is None:
            raise RuntimeError("Promoted advancedAudioControl widget not found in main window UI")
        self.volume_controls.initialize_audio(audio)
    ...
    def refresh_participants(self, silent: bool):
        logging.getLogger(__name__).debug("refresh_participants(silent=%s) entered", silent)
        logging.getLogger(__name__).debug("refresh_participants fetching peers")
        peers = self.discovery.peers()
        logging.getLogger(__name__).debug("refresh_participants got %d peers", len(peers))
        participants = [p["id"] for p in peers]
        logging.getLogger(__name__).debug("refresh_participants built participants=%s", participants)
        ...
        logging.getLogger(__name__).debug("refresh_participants(silent=%s) completed", silent)
    ...
    def update_live_ui(self):
        logging.getLogger(__name__).debug("update_live_ui entered")
        ...
        self_state = self.audio.capture_active
        row = self.rows.get(self.my_id)
        if row:
            row.setVolume(mic_level)
            row.setMicStatus(self_state)
        ...
        logging.getLogger(__name__).debug("update_live_ui completed")
...
def run_app():
    ...
    # Delay importing the Cython extension until after DLL search paths are configured.
    ...
    form_layout = dialog.findChild(QtWidgets.QFormLayout, "formLayout")
    security_edit = None

    if local_ip_label:
        local_ip_label.setText(get_local_ip())
    if form_layout is not None:
        security_label = QtWidgets.QLabel("Security code:", dialog)
        security_label.setObjectName("securityCodeLabel")
        security_edit = QtWidgets.QLineEdit(dialog)
        security_edit.setObjectName("securityCodeEdit")
        security_edit.setEchoMode(QtWidgets.QLineEdit.EchoMode.Password)
        form_layout.insertRow(2, security_label, security_edit)
    ...
    room_passphrase = security_edit.text().strip() if security_edit is not None else ""
    if not room_passphrase:
        logging.getLogger(__name__).info("Security code was empty; exiting.")
        sys.exit(0)

    logging.getLogger(__name__).info("Initializing audio/discovery stack...")
    logging.getLogger(__name__).info("Creating PyAudioEngine (thread id=%d)", threading.get_ident())
    audio = PyAudioEngine()
    logging.getLogger(__name__).info("PyAudioEngine created (thread id=%d)", threading.get_ident())
    discovery = PyPeerDiscovery()
    logging.getLogger(__name__).info("PyPeerDiscovery created (thread id=%d)", threading.get_ident())
    win = MainWindow(my_id, room_name, audio, discovery)
    win.show()

    def finish_startup() -> None:
        logger = logging.getLogger(__name__)
        logger.info("Applying saved audio settings on event loop (thread id=%d)", threading.get_ident())
        apply_saved_audio_settings(audio)
        logger.info("Applied saved audio settings (thread id=%d)", threading.get_ident())
        audio.set_room_secret(room_passphrase)
        logger.info("Set room secret on audio (thread id=%d)", threading.get_ident())
        discovery.start(my_id, audio.port(), room_name)
        logger.info("Discovery started (thread id=%d)", threading.get_ident())
        win.volume_controls.refresh_from_settings()
        win.refresh_participants(False)

    QtCore.QTimer.singleShot(0, finish_startup)
