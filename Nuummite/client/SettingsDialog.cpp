...
void SettingsDialog::applySavedDevicesToEngineBestEffort() {
    ...
    if (inIdx) {
        ...
        const bool exists = std::any_of(inputs.begin(), inputs.end(), [idx = *inIdx](const AudioDeviceInfo& dev) {
            return dev.index == idx;
        });
        (void)audio_->setInputDevice(exists ? *inIdx : -1);
    }
    if (outIdx) {
        const auto outputs = audio_->listOutputDevices();
        const bool exists = std::any_of(outputs.begin(), outputs.end(), [idx = *outIdx](const AudioDeviceInfo& dev) {
            return dev.index == idx;
        });
        (void)audio_->setOutputDevice(exists ? *outIdx : -1);
    }
}
